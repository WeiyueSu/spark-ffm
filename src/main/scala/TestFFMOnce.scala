...
object TestFFMOnce extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    val train_data = FFMLoader.load_ffm(sc, "/user/gzsuweiyue/Data/netease_ctr/ffm/train_n100xp_balance.ffm", doShuffle=args(11).toBoolean).repartition(partition_num)
    val valid_data = FFMLoader.load_ffm(sc, "/user/gzsuweiyue/Data/netease_ctr/ffm/valid_n100xp_balance.ffm", doShuffle=args(11).toBoolean).repartition(partition_num)
    ...
    val ffm: FFMModel = FFMWithAdag.train(train_data, m, n, dim = (args(5).toBoolean, args(6).toBoolean, args(1).toInt), n_iters = args(2).toInt,
      ...
      "adag", initWeights, Some(valid_data), miniBatchFraction = args(12).toDouble)
