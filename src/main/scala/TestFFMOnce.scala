...
object TestFFMOnce extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    val train_data = FFMLoader.load_ffm(sc, "/user/gzsuweiyue/Data/netease_ctr/ffm/train_n100xp_balance.ffm", doShuffle=true).repartition(partition_num)
    val valid_data = FFMLoader.load_ffm(sc, "/user/gzsuweiyue/Data/netease_ctr/ffm/valid_n100xp_balance.ffm", doShuffle=true).repartition(partition_num)
