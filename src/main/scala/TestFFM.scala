...
object TestFFM extends App {
  ...
  def train_one_day(sc: SparkContext, args: Array[String], date: String, initWeights: Option[Vector], m: Int, n: Int): FFMModel = {
    ...
    val pre_path: String = "/user/gzsuweiyue/Data/netease_ctr/txt/last_data/"
    val partition_num = args(7).toInt
    val train_data = FFMLoader.load_ffm(sc, pre_path + args(0), args(13).toDouble != args(14).toDouble, args(11).toBoolean, (args(13).toDouble, args(14).toDouble), setOne, dropTime=dropTime).repartition(partition_num).cache()
    //println("train cnt: ", train_data.count())
    //println("train positive", train_data.filter(x => x._1 == '1').count())
    
    val valid_data = if(args(8) != "None") Some(FFMLoader.load_ffm(sc, pre_path + args(8), args(13).toDouble != args(14).toDouble, false, (args(13).toDouble, args(14).toDouble), setOne, dropTime=dropTime).repartition(partition_num).cache()) else None
    ...
    val ffm: FFMModel = FFMWithAdag.train(train_data, m, n, dim=(args(5).toBoolean, args(6).toBoolean, args(1).toInt), n_iters=args(2).toInt,
      ...
      args(17), initWeights, valid_data, miniBatchFraction=args(12).toDouble, redo=(args(15).toDouble, args(16).toDouble))

    val modelPath = args(18)
    ffm.save(sc, modelPath)
    println("Save Done")

    val test_data = FFMLoader.load_ffm(sc, pre_path + args(9), setOne=setOne, dropTime=dropTime).repartition(partition_num).cache()
